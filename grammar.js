module.exports = grammar({
  ...
  rules: {
    program: ($) =>
      seq(
        repeat($.prql),
        repeat(choice($.pipeline, $.function_definition, $.variable))
      ),
    ...
    prql: ($) =>
      seq(
        ...
        choice(
          ...
          seq($.keyword_target, ":", field("target", $.target))
        )
      ),
    ...
    pipeline: ($) => seq(choice($.from, $.from_text), optional($.transforms)),
    ...
    variable: ($) =>
      seq(
        ...
        "=",
        choice(parens($.pipeline), seq($.from_text, optional($.transforms)))
      ),
    ...
    parameter: ($) =>
      choice($.identifier, seq($.identifier, ":", field("default", $.literal))),
    ...
    from_text: ($) =>
      seq(
        $.keyword_from_text,
        optional(
          seq($.keyword_format, ":", choice($.keyword_csv, $.keyword_json))
        ),
    ...
    from: ($) => seq($.keyword_from, $._table_selection),

    _table_selection: ($) =>
      choice(
        $._table_reference,
        seq(field("alias", $.identifier), "=", $._table_reference)
      ),

    _table_reference: ($) => choice(field("table_reference", $.identifier)),
    ...
    derives: ($) =>
      seq(
        ...
        choice(
          ...
          $.assignment,
          sq_bracket_list(
            choice($.term, seq("(", $.binary_expression, ")")),
            false
          )
        )
      ),
    ...
    append: ($) =>
      seq(
        choice($.keyword_append, $.keyword_remove, $.keyword_intersect),
        $._table_selection
      ),
    ...
    case: ($) =>
      seq($.keyword_case, sq_bracket_list(repeat1($.case_condition))),

    case_condition: ($) =>
      seq(
        choice($.binary_expression, field("default", $.literal)),
        "=>",
    ...
    aggregate_operation: ($) => seq($._agg_keywords, $._expression),

    _aggregate_func: ($) =>
      prec(
        1,
        seq(
          choice($.keyword_count, $.keyword_count_distinct, $.keyword_rank),
    ...
    sorts: ($) =>
      seq(
        $.keyword_sort,
        choice(sq_bracket_list($._sort_instruction, false), $._sort_instruction)
      ),

    _sort_instruction: ($) => seq(optional($.direction), $.identifier),

    direction: ($) => prec.left(choice("-", "+")),

    takes: ($) => seq($.keyword_take, choice($.integer, $.range)),

    window: ($) =>
      seq($.keyword_window, $.window_definitions, "(", $.derives, ")"),

    window_definitions: ($) => repeat1($._window_definition),

    _window_definition: ($) =>
      seq(
        choice($.keyword_rolling, $.keyword_rows, $.keyword_expanding),
        ":",
        choice($.range, $.integer)
      ),

    group: ($) =>
      seq(
        $.keyword_group,
        choice(sq_bracket_list($.term), $.term),
        "(",
        repeat1(choice($.aggregate, $.sorts, $.takes, $.window)),
        ")"
      ),
    ...
    _join_definition: ($) =>
      seq(
        seq($.keyword_side, ":"),
        choice($.keyword_inner, $.keyword_left, $.keyword_right, $.keyword_full)
      ),

    conditions: ($) =>
      choice(sq_bracket_list($.binary_expression, false), $._self_join),
    ...
    term: ($) =>
      prec(
        1,
        choice(field("value", $._expression), field("value", $.assignment))
      ),
    ...
    f_string: ($) => choice($._double_f_string, $._triple_f_string),

    s_string: ($) => choice($._double_s_string, $._triple_s_string),
    ...
    literal_string: ($) =>
      prec(1, choice($._single_quote_string, $._double_quote_string)),
    ...
    field: ($) =>
      prec(
        2,
        seq(
          optional(seq(field("table", $._alias_identifier), ".")),
          field("name", $.identifier)
        )
      ),

    _alias_identifier: ($) =>
      choice($.identifier, alias($._double_quote_string, $.identifier)),

    identifier: ($) => choice($._identifier, seq("`", $._identifier_dot, "`")),
    _identifier: (_) => /([a-zA-Z_][0-9a-zA-Z_]*)/,
    _identifier_dot: (_) => /([a-zA-Z_.][0-9a-zA-Z_.]*)/,

    range: ($) =>
      choice(
        ...[["..", "range"]].map(([operator, range_prec]) =>
          prec.left(
            range_prec,
            choice(
              seq(
    ...
    _agg_rhs_assignment: ($) =>
      prec(
        1,
        field("operation", seq(optional($._agg_keywords), $._expression))
      ),

    assignment: ($) =>
      choice(
        ...[["=", "alias_assignment"]].map(([operator, range_prec]) =>
          prec.left(
            range_prec,
            choice(
              seq(
    ...
    timestamp: ($) =>
      prec.right(seq("@", $._date, "T", $._time, optional($.timezone))),

    timezone: ($) => choice("Z", seq($.direction, /\d{1,2}/)),
...
function comma_list(field, requireFirst) {
  ...
  return optional(seq(field, repeat(seq(",", field))));
}
...
function paren_list(field, requireFirst) {
  return seq("(", comma_list(field, requireFirst), ")");
}

function sq_bracket_list(field, requireFirst) {
  return seq("[", comma_list(field, requireFirst), optional(","), "]");
}
